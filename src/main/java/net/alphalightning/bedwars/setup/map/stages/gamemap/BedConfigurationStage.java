...
public class BedConfigurationStage extends Stage implements TeamConfiguration, LocationConfiguration {
    ...
    private void updateBed(MapSetup setup, Location bottom) {
        ...
        this.visualizationManager.registerTask(setup, new MultiBlockRenderer(plugin, List.of(topHalf.getBlock(), bottom.getBlock())).render(new MultiBlockVisualization(team.color())));
        this.visualizationManager.registerTask(setup, new FakeBlockRenderer(plugin, bottom).render(new FakeBlockVisualization(player, BedUtils.fromColor(team.color()))));
